(() => {
  ...
  function overviewProcessData(process) {
    ...
    // Todas as regionais selecionadas: o Brasil DDA também inclui Grupo Network,
    // que deliberadamente não possui linha regional ou de grupo.
    const wholeBrazil = state.data.regions.every((region) => state.overviewRegions.has(region));
    const total = wholeBrazil ? overviewAggregates(process, 'Brasil')[0] || null : regional.length ? combine(regional, process) : null;
    return {total, regions:regional, groups:overviewAggregates(process, 'Grupo')};
  }
  ...
  function selectedTotal() {
    ...
    // DDA e BC incluem Grupo Network no consolidado Brasil: o filtro de
    // regionais altera apenas as linhas regionais, nunca a linha Brasil.
    if (state.process === 'dda' || state.process === 'battery_check') return activeRows('Brasil')[0] || null;
